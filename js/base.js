...
//图片组合
(function() {
    var oUl = document.getElementById("tabShow");
    var aLi = oUl.children;
    var oTabBtn = document.getElementById("tabBtn5");
    var aBtn = oTabBtn.children;
    function direction(obj, oEvent) {
        var x = oEvent.clientX - getPos(obj).left - obj.offsetWidth / 2;
        var y = obj.offsetHeight / 2 + getPos(obj).top - oEvent.clientY;
        var a = Math.atan2(y, x);
        return Math.round((a * 180 / Math.PI + 180) / 90) % 4
    }
    var oLi = document.getElementById("show");
    var aLi = oLi.children;
    var len = aLi.length;
    var zIndex = 1;
    var aPos = [];
    for (var i = 0; i < len; i++) {
